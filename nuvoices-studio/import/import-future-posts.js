/**
 * Import 6 WordPress posts that had status "future" (scheduled) at the time of
 * the XML export and were skipped during the original Sanity migration.
 *
 * Usage:
 *   node import-future-posts.js            # dry run — preview only
 *   node import-future-posts.js --commit   # actually write to Sanity
 *   node import-future-posts.js --commit --skip-images
 */
...
// ---------------------------------------------------------------------------
// 1. Hardcoded category assignments for the 6 "future" posts
// ---------------------------------------------------------------------------
const FUTURE_POSTS = [
  { wpId: 4190, category: 'magazine', subCategories: ['personal-essay'] },
  { wpId: 4193, category: 'magazine', subCategories: ['personal-essay'] },
  { wpId: 4218, category: 'magazine', subCategories: ['film'] },
  { wpId: 4220, category: 'magazine', subCategories: ['film'] },
  { wpId: 4222, category: 'magazine', subCategories: ['film'] },
  { wpId: 4272, category: 'magazine', subCategories: ['profile'] },
];
...
// ---------------------------------------------------------------------------
async function importFuturePosts(options = {}) {
  const { commit = false, skipImages = false } = options;

  console.log(`\n=== Import Future WordPress Posts ===`);
  ...
  console.log(`Posts: ${FUTURE_POSTS.length}`);
  console.log(`Skip images: ${skipImages}\n`);

  // Fetch all 6 posts from WP REST API in one request
  const ids = FUTURE_POSTS.map((p) => p.wpId).join(',');
  const wpApiUrl = `https://nuvoicesprod.wpenginepowered.com/wp-json/wp/v2/posts?include=${ids}&per_page=${FUTURE_POSTS.length}&_embed`;
  ...
  for (const wpPost of wpPosts) {
    ...
    try {
      ...
      if (commit) {
        ...
        console.log(`    featured image: ${featuredUrl ? 'yes' : 'no'}`);
...
if (require.main === module) {
  ...
  importFuturePosts({ commit, skipImages })
    .then((result) => {
...
module.exports = importFuturePosts;
